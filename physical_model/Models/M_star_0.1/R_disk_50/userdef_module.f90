...
doubleprecision :: userdef_freqs, userdef_flux, userdef_flux_int
integer :: userdef_nfreq, userdef_inu, userdef_i
...
subroutine userdef_setup_model()
  ...
  integer :: userdef_nfreq, userdef_inu, ierror, userdef_i

  call read_mc_frequencies()
  userdef_freqs(:) = mc_frequencies(:)
  userdef_nfreq = mc_nrfreq
  allocate(userdef_flux_int(:,:))
  userdef_flux_int = 0
  deallocate(userdef_flux_int)
  do userdef_inu=1,userdef_nfreq-1
    write(*,*) userdef_freqs(userdef_inu)
    mc_frequencies(1) = userdef_freqs(userdef_inu)
    mc_nrfreq = 1
    !
    ! Now call the monochromatic Monte Carlo
    !
    call do_monte_carlo_scattering(rt_mcparams,ierror,resetseed=do_resetseed,meanint=.true.)
    !now caculate flux
    allocate(userdef_flux(userdef_inu,:))
    userdef_flux = mcscat_meanint(1,:) * userdef_freqs(userdef_inu) * (userdef_freqs(userdef_inu+1)- userdef_freqs(userdef_inu))
    deallocate(userdef_flux)
    allocate(userdef_flux_int(userdef_inu,:))
    userdef_flux_int = userdef_flux_int(userdef_inu,:) + userdef_flux(userdef_inu,:)
    deallocate(userdef_flux_int)
  enddo

  open(unit=20, file='total_flux.out', status='replace', action='write')
